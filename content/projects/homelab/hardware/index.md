...
My homelab currently consists of three computing devices connected through an unconventional networking setup. I have a NAS, a Proxmox server, and a Raspberry Pi. This has been built for nearly free over a couple of years.

## NAS

I built a NAS in an old Dell OptiPlex with a Pentium G630 and 4GB of ram. I got this for free from a relative's workplace. As far as I can tell, it was used to run a photo booth, because when I got it, it had Windows 7 installed on the 265gb HDD with a photo booth user and some errors from missing hardware.

I have since installed OpenMediaVault on an SSD and connected two 1TB laptop hard drives in a ZFS pool. I am only using about 250GB at the moment, but I want to upgrade the drives to have at least four or eight terabytes of total storage soon, so that I can store future video footage and expand my movie collection. Unfortunately the OptiPlex only has 3 SATA ports, so using more than two data drives will require booting from a USB drive or replacing the OptiPlex entirely.
...
I have Proxmox running on a battered old gaming laptop. I got it for free from a local buy nothing group, and used it as my main desktop computer for about four years before I built my current pc. The hinges were broken when I got it, and the keyboard has lost multiple keys over time. The forward slash (`/`) key likes to randomly register key presses, so I keep the keyboard disabled.
